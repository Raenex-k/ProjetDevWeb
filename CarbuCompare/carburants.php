<?php
// Page "Comparateur" : carte des regions -> departement -> ville -> prix
...
if ($recherche !== '' && strlen($recherche) >= 2) {
    ...
    if (count($resultats_recherche) === 1) {
        ...
        // Redirection vers la page avec les 3 parametres pre-remplis
        header("Location: carburants.php?region={$region_direct}&dep={$dep_direct}&ville={$insee_direct}");
        exit;
    }
}

// Etape 1 : regions metropolitaines
$regions = [];
$f = fopen(__DIR__ . '/data/regions.csv', 'r');
fgetcsv($f, 0, ',', '"', '\\');
while (($l = fgetcsv($f, 0, ',', '"', '\\')) !== false) {
    if ((int) $l[0] >= 11) $regions[$l[0]] = $l[5];
}
fclose($f);

// Etape 2 : departements de la region choisie
$departements = [];
if (isset($regions[$region])) {
    $f = fopen(__DIR__ . '/data/departements.csv', 'r');
    fgetcsv($f, 0, ',', '"', '\\');
    while (($l = fgetcsv($f, 0, ',', '"', '\\')) !== false) {
        if ($l[1] === $region) $departements[$l[0]] = $l[5];
    }
    fclose($f);
}

// Etape 3 : villes du departement choisi
$villes = [];
...
// Etape 4 : stations autour de la ville choisie
$infos_ville = null;
$stations    = [];
    ...
    <form method="get" action="carburants.php" class="filtres">
        <div class="filtre">
            ...
            <input type="text" id="recherche" name="recherche"
                   ...
                   placeholder="ex. Cergy, Paris, Lyon..." />
        </div>
...
<div class="contenu">
    ...
    <!-- Etape 1 : carte cliquable des regions -->
...
<div class="etape-region">
    ...
    <!-- Etape 4 : prix des stations autour de la ville -->
    <?php if ($infos_ville !== null) { ?>
        <h2>4. Prix a <?= clean($infos_ville['nom']) ?></h2>
        <p>Stations dans un rayon de 10 km autour de
           <strong><?= clean($infos_ville['nom']) ?> (<?= clean($infos_ville['cp']) ?>)</strong>.</p>
